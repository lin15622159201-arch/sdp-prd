...
sequenceDiagram
    ...
    participant Agent as Agent平台

    U->>FE: 勾选 SKC，点击「发送到」→「智能开款」
    FE->>FE: 前置校验（数量 ≤50 且不含已取消）
    FE->>FE: 打开弹窗（展示 skc_id + 营销图 list）
    FE->>Agent: 跳转/嵌入 Agent 界面（携带 SKC 列表）
    U->>Agent: 在 Agent 界面选择任务配置后确认提交
    Agent-->>FE: 返回 agent_task_id + task_channel
    FE->>BE: POST /skc-agent-task（skc_id[], agent_task_id, task_channel）
...
sequenceDiagram
    ...
    U->>FE: 提交创建开款任务
    FE->>BE: POST /design-task（含 skc_id[]）
    BE->>BE: 创建开款任务（taskSource=AGENT_DEVELOP）
    BE-->>FE: 任务创建成功（主流程完成）
    Note over BE: 以下为异步流程，不阻断主流程
    BE->>BE: 读取关联 SKC 的 develop_tags[]
    alt SKC 存在开款标签
        BE->>PLM: 推送开款标签列表（skc_id + tags[]）
        alt 推送成功
            PLM-->>BE: 200 OK
            BE->>BE: 记录推送日志：成功
        else 推送失败
            PLM-->>BE: 错误响应
            BE->>BE: 记录推送日志：失败，加入重试队列
        end
    else SKC 无开款标签
        BE->>BE: 跳过推送
    end
...
| 页面名称 | 变更类型 | 具体变更 | 变更原因 |
|---------|---------|---------|---------|
| 款式列表 | 修改 | 查询条件新增「智能编辑任务」多选下拉（未发起/进行中/待审核/已完成/异常） | 支持按 Agent 任务状态过滤 |
| 款式列表 | 修改 | 查询条件新增「开款标签」多选筛选 | 按 Agent 开款标签过滤款式 |
| 款式列表 | 修改 | SKC 行显示列新增「关联任务」列：聚合状态标签 + 悬浮展示任务列表（任务编号/类型/状态），默认待审核任务置顶；含【审核】按钮（存在待审核任务时可操作，跳转对应任务审核界面）；任务编号可点击跳转任务中心列表并筛选对应任务 | 在列表直接追踪和审核 Agent 任务 |
| 款式列表 | 修改 | SKC 行显示列新增「开款标签」标签展示 | 展示 Agent 推送的开款标签 |
| 款式列表 | 修改 | 工具栏新增「同步任务状态」按钮，点击触发当前页所有 SKC 关联 Agent 任务状态的手动同步 | 弥补 Agent 回调延迟 |
| 款式列表 | 修改 | 「发送到」下拉新增【智能开款】选项；前置条件：选中数量 ≤50 条，且所选数据不包含已取消状态 | 接入 Agent 智能开款入口 |
| 发送到智能开款 | 新增 | 新增弹窗页面：展示选中 SKC 列表（SKC id + 营销图 list）；用户在 Agent 界面选择对应任务后提交至 SDP 后端；SDP 保存 skc_id + agent任务ID + 任务渠道；提交成功后刷新列表并停留在当前页/位置 | 智能开款任务发起入口 |
...
| 页面名称 | 变更类型 | 具体变更 | 变更原因 |
|---------|---------|---------|---------|
| 现货管理-列表 | 修改 | 查询条件新增「智能编辑任务」多选下拉（未发起/进行中/待审核/已完成/异常） | 同款式管理 |
| 现货管理-列表 | 修改 | SKC 行新增「关联任务」列：聚合状态标签 + 悬浮任务列表，默认待审核置顶；含【审核】跳转按钮和任务编号跳转链接 | 同款式管理 |
| 现货管理-列表 | 修改 | 工具栏新增「同步任务状态」按钮 | 同款式管理 |
| 现货管理-列表 | 修改 | 批量操作「发送到」新增【智能开款】选项；前置条件：选中 ≤50 条且不含已取消状态 | 接入 Agent 智能开款入口 |
| 发送到智能开款 | 新增 | 弹窗交互规格同款式管理，复用相同弹窗规格 | 智能开款任务发起入口 |
...
任务来源枚举「以料开款」修改为「智能开款」；开款任务列表新增任务来源多选筛选条件。

#### 页面变更

| 页面名称 | 变更类型 | 具体变更 | 变更原因 |
|---------|---------|---------|---------|
| 开款任务列表 | 修改 | 查询条件新增「任务来源」多选筛选（手动创建/Excel导入/AIGC选款/智能开款/…） | 区分 Agent 推送的开款任务与其他来源 |
