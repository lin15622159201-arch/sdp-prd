...
| 8 | 开款任务管理：新增供 Agent 系统调用的创建开款任务接口（字段等同前端创建）；在已支持字段（图片≤10张、店铺必填）基础上，新增通用必填：款式标签、波段；非现货款必填：项目类型、前置拆版；可选：花型源文件URL | 新增功能 | P0 |
...
sequenceDiagram
    ...
    Agent->>BE: POST /agent-callback（agent_task_id, task_status, result_images[]）
    BE->>BE: 查找关联 skc_agent_task 记录
    BE->>BE: 更新 task_status + updated_at
    alt task_status = 4（已完成）且 result_images 非空
        BE->>BE: 查询当前 SKC 营销图列表（marketing_images）
        BE->>BE: 计算可追加数量 = max(0, 10 - 当前营销图数量)
        BE->>BE: 取 result_images 前 N 张（N = 可追加数量），追加到营销图列表；超出10张的部分忽略
        BE->>BE: 保存更新后的营销图列表
    end
    BE-->>Agent: 更新成功
...
#### Flow 4：Agent 系统创建开款任务

```mermaid
sequenceDiagram
    participant Agent as Agent平台后端
    participant BE as SDP后端
    participant PLM as PLM（外部系统）

    Agent->>BE: POST /design-task/agent-create（见接口字段定义）
    BE->>BE: 参数校验（通用必填 + 非现货款条件必填）
    alt 校验失败
        BE-->>Agent: 400 参数错误 + 错误明细
    end
    BE->>BE: 创建开款任务记录（taskSource=AGENT_DEVELOP）
    BE-->>Agent: 201 任务创建成功（返回 task_id + task_code）
```
---
...
8. **款式列表** `修改` — 新增逻辑，从agent已完成的任务获取营销图
...
| Agent 回调写入营销图 | Agent 任务回调时，若 `task_status=4`（已完成）且回调携带 `poses`字段，则以**增量追加**方式更新关联 SKC 的营销图：取 `result_images` 中前 N 张追加（N = max(0, 10 - 当前营销图数量)），超出10张上限的部分忽略 |
...
`POST /design-task/agent-create`
v2.6.2 提供给agent系统创建开款任务的接口，需要新增字段（等同于前端创建）
已支持字段：
...
|------|------|---------|----------|------|
| shop_id | 整数 | 必填 | — | 店铺 ID |
| images | 字符串数组 | 必填 | 1–10 张，每条 URL ≤500字符 | 营销图 URL 列表 |
| develop_tags | 字符串数组 | 可选 | 最多20个，每个标签 ≤50字符 | 开款标签，由 Agent 系统定义并传入 |
| style_tags | 字符串数组 | 必填 | 仅1个，从字典下拉选择 | 款式标签 |
| band | 短文本 | 必填 | ≤32字符 | 波段 |
| project_type | 枚举 | 非现货款必填，现货款忽略 | — | 项目类型，枚举值与前端一致 |
| pre_split | 布尔 | 非现货款必填，现货款忽略 | — | 是否前置拆版 |
| pattern_source_url | 字符串 | 可选 | ≤500字符 | 花型源文件 URL |
| clothType | 枚举 | 必填 | — | AI_STYLE，SPOT_STYLE；决定条件必填字段的校验范围 |
...
|------|------|------|
| task_id | 整数 | 新建任务主键 |
| task_code | 短文本 | 任务编号，用于展示和跳转 |

**错误码**

| HTTP 状态 | code | 场景 |
|----------|------|------|
| 400 | MISSING_REQUIRED_FIELD | 通用必填字段缺失 |
| 400 | MISSING_NON_SPOT_FIELD | 非现货款条件必填字段缺失（project_type / pre_split）|
| 400 | IMAGE_LIMIT_EXCEEDED | 图片数量超过 10 张 |
| 400 | INVALID_SHOP | shop_id 不存在 |

#### 业务规则变更

| 规则 | 内容 |
|------|------|
| 条件必填校验 | `clothType=AI_STYLE` 时，`project_type` 与 `pre_split` 为必填；缺失时返回 400 `MISSING_NON_SPOT_FIELD` |
| 任务来源标记 | 通过此接口创建的任务 `taskSource` 固定写入 `AGENT_DEVELOP` |
| 图片上限 | `images` 最多 10 条，超出返回 400 `IMAGE_LIMIT_EXCEEDED` |
| 花型源文件 | `pattern_source_url` 为可选字段，传入时存储，不传时留空 |
