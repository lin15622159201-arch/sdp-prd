...
- module: shop-management
  type: hard
  use_case: 选择店铺后读取默认风格自动填充 (v2.6.0 变更)
...
| 编辑开款任务 | 表单页 | 开款任务列表 → 点击「编辑」 | 页面跳转 | 修改已创建的开款任务，支持修改款式类型、店铺等信息 (v2.6.0 新增) |
...
| notes | 长文本 | 否 | — | ≤2000字符 | 备注，设计师填写供审核参考 (v2.6.0 新增) |
| projectType | 引用 | 条件必填 | — | — | 项目类型，非现货款时必填，取值来源：NEST字典 project_type (v2.6.0 新增) |
| preSplitTag | 引用 | 否 | — | — | 前置拆版标签，取值来源：NEST字典 pre_split_tag (v2.6.0 新增) |
| productionMethod | 枚举 | 否 | — | — | 制作方式，枚举值：3D / 实物样 (v2.6.0 新增) |
...
**用例8：编辑开款任务 (v2.6.0 新增)**
- **前置条件**：1）需要有对应权限；2）开款状态taskStatus 为待审核的数据
- **操作**：打开编辑页面
- **后置输出**：记录编辑日志
...
| 项目类型 | 单选 | 条件必填 | — | — | 取值来源：NEST字典 project_type。非现货款时必填，现货款(SPOT_STYLE)时不显示/非必填。支持批量填写 (v2.6.0 新增) | |
| 前置拆版标签 | 单选 | 条件必填 | 非现货款时必填 | — | 取值来源：NEST字典 pre_split_tag。支持批量填写 (v2.6.0 新增) | |
| 备注 | 长文本 | 否 | ≤2000字符 | — | 设计师填写供审核参考 (v2.6.0 新增) | |


### 业务规则 (v2.6.0 新增)

- **#156 项目类型条件必填**: 项目类型在非现货款(SPOT_STYLE)时必填；现货款时不显示该字段或为非必填。
- **#125 多店铺开款**: 支持批量提交不同店铺的开款任务；批量导入后可在编辑页单独编辑每个款式的店铺信息。
- **#154 清除空白行**: 创建款式后，自动清除无图片且无任何信息填写的空白款式行。
...
**编辑范围 (#74 v2.6.0 新增)**: 支持修改任务中除款式图片以外的所有字段，包括款式类型、款式标签等。

### 可编辑字段

| 字段名称 | 输入方式 | 必填 | 校验规则 | 说明 | 更新说明 |
|---------|---------|------|---------|------|------|
| 款式类型(designTypeCode) | 单选 | 否 | — | 取值来源：NEST字典 styletype。#74 支持修改款式类型 (v2.6.0 新增) | |
| 款式标签 | 单选 | 否 | — | 取值来源：NEST字典 product_tag。#74 支持修改款式标签，变更后款式级别自动重新映射 (v2.6.0 新增) | |
| 店铺(storeId) | 单选 | 是 | — | 取值来源：POP-店铺管理。#125 批量导入后支持单独编辑店铺 (v2.6.0 新增) | |
| 波段 | 单选 | 否 | — | 取值来源：NEST字典 plm_clothing_band | |
| 平台 | 单选 | 否 | — | 取值来源：NEST字典 material_source | |
| 价格 | 数字输入 | 否 | 0-9999.99，精度2位 | | |
| 供应商 | 文本输入 | 否 | ≤20字符 | | |
| 供应商款号 | 文本输入 | 否 | ≤20字符 | | |
| 商品链接 | 文本输入 | 否 | ≤500字符 | | |
| 项目类型 | 单选 | 条件必填 | — | 取值来源：NEST字典 project_type。非现货款时必填 (v2.6.0 新增) | |
| 前置拆版标签 | 单选 | 条件必填 | 非现货款时必填 | 取值来源：NEST字典 pre_split_tag (v2.6.0 新增) | |
| 备注 | 长文本 | 否 | ≤2000字符 | (v2.6.0 新增) | |


### 用例

**用例1：编辑开款任务**
- **前置条件**：1）需要有编辑权限；2）taskStatus IN (0, 10)
- **操作**：在列表页操作列点击「编辑」，页面跳转到编辑表单页，加载当前任务数据
...
- 保存后更新任务数据，状态不变
**后置输出**：
修改后保存，更新开款任务信息，返回列表页

### 异常处理

| 异常场景 | 用户看到的表现 |
|---------|-------------|
| 状态不允许编辑 | 提示"当前状态不支持编辑"，按钮不显示 |
| 权限不足 | 按钮不显示 |
...
- 每张卡片显示：
  ...
  - 设计师、波段、店铺、平台、价格信息
  - 项目类型、前置拆版标签 (v2.6.0 新增 #231)
  - 备注内容（设计师填写的备注，长文本展示）(v2.6.0 新增 #142)
...
| v2.6.0 | 审核信息增强 | (v2.6.0 新增) 审核卡片展示备注(#142)、项目类型、前置拆版标签(#231) |
...
| 项目类型 | 单选 | 条件必填 | 取值来源：NEST字典 project_type。非现货款时必填，可批量填写 (v2.6.0 新增 #231) |
| 前置拆版标签 | 单选 | 条件必填 | 取值来源：NEST字典 pre_split_tag。非现货款时必填，可批量填写 (v2.6.0 新增 #231) |
| 制作方式 | 单选 | 否 | 枚举值：3D / 实物样。可批量填写 (v2.6.0 新增 #231) |
| 是否拼接 | 开关 | 否 | 是否拼接处理。可批量填写 (v2.6.0 新增 #231) |
| 尺码 | 单选 | 否 | 取值来源：NEST字典 size。可批量填写 (v2.6.0 新增 #231) |

### 批量填写

批量填写功能通过表头下拉选择器实现，选择后批量覆盖款式行。

**批量填写规则 (v2.6.0 新增 #207)**: 批量填写仅覆盖空白/未填写的字段，保留已填写的数据，避免误覆盖。若需要强制覆盖，用户需先手动清空目标字段。

**批量填写分组**：

| 分组 | 包含字段 |
|------|---------|
| 企划信息 | 品类、颜色、波段、店铺、款式类型、款式标签、款式级别、项目类型、前置拆版标签 |
| 销售属性 | 视觉形式、风格、季节、节日、面料池使用范围、印花类型、版型、弹性 |
| SKU 分类 | SKU 分类、套装件数/单品数量 |
| 生产标签 | 尺码组、织造方式、品质等级、制作方式、是否拼接、尺码 |
...
| 店铺 | 选择店铺 | 风格(clothingStyleCode) | 自动填充 | 读取所选店铺的默认风格自动回填 (v2.6.0 新增 #145) |
| 款式标签 | 选择标签 | 款式级别(styleLevelCode) | 自动映射 | 根据款式标签自动映射对应款式级别 (v2.6.0 新增 #145) |
| 品类(套装) | 选择套装品类 | SKU 分类件数(suitPiece) | 自动填充 | 根据所选品类自动填充 SKU 单品数量 (v2.6.0 新增 #145) |
...
- 点击图片可进入全屏预览大图，点击图片浏览的空白区域退出浏览模式 (v2.6.0 新增 #181)

### 提交说明

**提交时校验**:
1. 必填项：品类、颜色、店铺、款式标签、款式级别、视觉形式、风格、季节、印花类型、版型、弹性、尺码组、织造方式、品质等级
2. 非现货款时额外校验：项目类型、前置拆版标签 为必填 (v2.6.0 新增 #231)
3. 校验失败时提示"第 X 行数据存在必填项未填写"，并自动滚动到第一个校验失败的行 (v2.6.0 新增 #153)
4. 现货款 (SPOT_STYLE) 仅取第一个颜色和第一个 SKC 提交
...
| v2.6.0 | 批量填写优化+校验滚动+字段新增 | (v2.6.0 新增) 批量填写仅覆盖空白字段(#207)；校验失败自动滚动到第一个错误行(#153)；新增项目类型、前置拆版标签、制作方式、是否拼接、尺码字段，非现货款时项目类型和前置拆版标签为必填(#231) |
...
| v2.6.0 | 编辑开款任务+字段联动映射+项目类型/前置拆版标签/备注/开发信息/制作方式+批量填写仅覆盖空值+校验失败自动滚动 | 来自规划文档 v2.6.0：新增编辑开款任务页面（支持修改款式类型#74、批量导入后编辑店铺#125）；创建页面新增字段联动映射规则（店铺→风格#145、款式标签→款式级别、品类→SKU件数）；新增项目类型（条件必填#156）、前置拆版标签（#231）、备注（#142）、开发信息（#175）、制作方式（#214）字段；图片浏览点击空白退出（#181）；创建后自动清除空白行（#154）；批量填写仅覆盖空值（#207）；校验失败自动滚动到错误行（#153）；审核页面显示备注、项目类型、前置拆版标签；数据模型新增 notes、projectType、preSplitTag、productionMethod、developmentInfo 字段；depends_on 更新 shop-management use_case |
