...
(function () {
  ...
  function highlightDocSection(ref, sticky) {
    ...
    const anchorN = normalize(anchor);

    // 1. Try headings first (h2/h3/h4)
    let target = null;
    const headings = content.querySelectorAll('h2,h3,h4');
    for (const h of headings) {
      const t = h.textContent;
      if (normalize(t).includes(anchorN) || h.id.includes(anchor)) {
        target = h;
        break;
      }
    }

    // 2. Fallback: search <strong> / <b> bold text (covers 用例N paragraphs)
    if (!target) {
      const bolds = content.querySelectorAll('strong, b');
      for (const b of bolds) {
        if (normalize(b.textContent).includes(anchorN)) {
          // Use the closest block ancestor (p, li, div) as target
          target = b.closest('p,li,div,td') || b;
          break;
        }
      }
    }

    // 3. Fallback: any element whose text contains the anchor
    if (!target) {
      const walker = document.createTreeWalker(content, NodeFilter.SHOW_TEXT);
      let node;
      while ((node = walker.nextNode())) {
        if (normalize(node.textContent).includes(anchorN)) {
          target = node.parentElement.closest('p,li,h2,h3,h4,div,td') || node.parentElement;
          break;
        }
      }
    }
