...
**入口**: 待上架列表 → 点击「编辑」（review_status=已通过 且 release_status=待发布/发布失败 的记录）
...
编辑页展示当前待上架记录的 SPU 信息和 SKC 子列表，允许对各字段进行修改，保存后更新 style_on_shelves 及 skc_on_shelves 记录。

### ✨ SKC 级别删除 (v2.8.0 新增)

仅对 `listing_type=2（二次上架）且 release_status=0（待发布）` 的记录开放 SKC 删除操作。

#### 触发条件

| 条件 | 说明 |
|------|------|
| listing_type | 必须为 2（二次上架） |
| release_status | 必须为 0（待发布） |
| SKC 数量 | 当前记录下 SKC 数量 > 1；仅剩 1 个时按钮 disabled |

#### 操作入口

SKC 子列表每行操作列显示「删除」按钮，满足以上触发条件时可点击。

#### 业务规则

1. 点击「删除」后弹出二次确认弹窗，提示「确认删除该 SKC？」
2. 确认后从 skc_on_shelves 中删除该 SKC 行记录（软删除或物理删除，由研发确认）
3. 不回调款式管理/现货管理源系统
4. 仅剩 1 个 SKC 时，「删除」按钮 disabled，tooltip 提示「至少保留一个 SKC，如需删除整条记录请返回列表操作」
5. 如需删除全部 SKC，须返回待上架列表使用整行删除操作

#### SKC 删除异常处理

| 异常场景 | 用户看到的表现 |
|---------|-------------|
| SKC 数量为 1，点击删除 | 按钮 disabled，不可操作 |
| 非二次上架或非待发布状态 | 「删除」按钮不显示 |
| 删除请求失败 | 提示"删除失败，请重试" |
...
| v2.8.0 | 二次上架删除 + SKC 级别删除 + 字段同步策略 | (v2.8.0 新增) 待上架列表支持删除二次上架待发布整行记录（软删除，不回调源系统）；编辑待上架页新增 SKC 级别删除操作（至少保留 1 个 SKC）；新增字段同步策略章节，定义款式管理/现货管理→待上架/商品的 SPU/SKC 各字段同步方式（直接修改/确认后可修改/修改后需通知/不可修改）；新增第 8 章编辑待上架 |
