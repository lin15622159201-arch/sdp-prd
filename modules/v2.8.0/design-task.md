...
| ✨ 提交任务 | `TJRW` | 显示「提交」按钮，仅待提交状态 + 创建人本人可操作 (v2.8.0 新增) |
| 删除任务 | `SCRW` | 显示「删除任务」按钮，仅待提交/待审核/待开款 + 创建人本人可操作 (v2.8.0 变更：新增待提交状态可删除) |
...
| 状态码 | 名称 | 说明 |
|-------|------|------|
| ✨ `-1` | 待提交 | 创建后初始状态，等待创建人提交 (v2.8.0 新增) |
| `0` | 待审核 | 创建人提交后进入，等待组长审核 |
...
> **待提交说明 (v2.8.0 新增)**: 任务创建后初始状态为「待提交」（状态码 `-1`），创建人可在此状态查看查重标签、删除任务，由创建人主动「提交」后才进入「待审核」流程。
>
> **删除说明**: 逻辑删除（设置 `deleted = true`），仅创建人可操作，支持待提交/待审核/待开款三个状态 (v2.8.0 变更：新增待提交状态)。
...
| taskStatus | 枚举 | 是 | -1 | — | -1-待提交 / 0-待审核 / 10-待开款 / 20-已淘汰 / 30-已开款 / 50-失败 (v2.8.0 变更：默认值由 0 改为 -1，新增待提交状态) |
...
| ✨ duplicateCheckLabel | 枚举 | 自动 | null | — | 查重标签：SAME（有同款）/ SIMILAR（有相似款）/ NONE（无同款相似款），任务创建后图片识别完成时同步写入，失败不影响主流程 (v2.8.0 新增) |
...
~~Tab 下方的状态标签从后端统计接口获取各状态数量。~~ 状态筛选由单选标签改为**多选下拉**，支持同时筛选多个状态。各状态选项从后端统计接口获取数量。可选状态：全部、✨ 待提交(n)、待审核(n)、待开款(n)、已开款(n)、已淘汰(n)。(v2.8.0 变更：新增「待提交」状态选项)
...
| ✨ 查重标签 | 单选 | 有同款 / 有相似款 / 无同款相似款 (v2.8.0 新增) |
...
| 开款信息 | 文本 | 品类 / 波段 / 店铺 |
| 状态 | 状态标签 | 待提交 / 待审核 / 待开款 / 已开款 / 已淘汰 (v2.8.0 变更：新增待提交) |
| AI 识别状态 | 状态标签 | 排队中 / 识别中 / 已中止 / 已完成 / 失败 |
| 重复款标识 (v2.4.1 新增) | 标签 | 显示「重复款」标签，当 duplicateFlag = true 时高亮展示 |
| ✨ 查重标签 (v2.8.0 新增) | 标签 | 根据 duplicateCheckLabel 显示：有同款（红色）/ 有相似款（橙色）/ 无同款相似款（灰色）；识别未完成时不显示 |
...
**用例5：删除任务 (v2.8.0 变更)**
- **前置条件**：1）需要有 SCRW 权限；2）至少选中一条数据；3）仅支持 taskStatus IN (-1, 0, 10)（待提交/待审核/待开款）的数据 (v2.8.0 变更：新增待提交状态可删除)；4）仅创建人可操作
- **操作**：点击「删除任务」按钮，弹出二次确认弹窗
- **确认提示**：「确认删除开款任务？删除后不可恢复」
- **后置输出**：确认后逻辑删除对应开款任务（设置 deleted = true）

✨ **用例10：提交任务 (v2.8.0 新增)**
- **前置条件**：1）需要有 TJRW 权限；2）至少选中一条数据；3）仅支持 taskStatus = -1（待提交）的数据；4）仅创建人可操作
- **操作**：点击「提交」按钮，弹出二次确认弹窗
- **确认提示**：「确认提交选中的开款任务？」
- **后置输出**：确认后将任务状态更新为待审核（taskStatus = 0）
...
| 删除状态不匹配 | 提示"请勾选待提交、待审核或待开款的数据进行删除操作" (v2.8.0 变更) |
...
**成功后数据变更**：保存后创建【待提交】状态的开款任务（taskStatus = -1），同步触发图片识别（异步），返回列表页 (v2.8.0 变更：初始状态由待审核改为待提交)
...
| v2.8.0 | 待提交状态+查重标签 | (v2.8.0 新增) 任务初始状态改为待提交（taskStatus = -1），由创建人主动提交后进入待审核；图片识别完成后写入查重标签（duplicateCheckLabel: SAME/SIMILAR/NONE），失败不影响主流程；列表新增查重标签筛选和显示列；新增提交任务权限码（TJRW）；删除权限新增待提交状态；数据模型新增 duplicateCheckLabel 字段 |

---

## 数据迁移说明（v2.8.0）

### taskStatus 默认值变更（0 → -1）

- **影响范围**：仅影响新创建的任务；历史存量任务 `taskStatus` 值不变（仍为 0/10/20/30/50），无需回填
- **兼容性**：前端列表、审款页入参校验需新增对 `-1`（待提交）状态的处理；状态筛选下拉新增「待提交」选项，旧选项不受影响
- **API 兼容性**：任务状态枚举新增 `-1` 值，接收方需适配该值，否则会将待提交任务误判为未知状态

### duplicateCheckLabel 新增字段

- **历史数据**：已存在的任务该字段默认为 `null`（无查重结果）；前端处理 `null` 时不显示查重标签，等同于「未查重」
- **回填策略**：不对历史数据做异步回填；仅新创建任务在图片识别完成后写入，写入失败不重试、不阻塞主流程
- **数据一致性**：`duplicateFlag`（原有布尔字段）与 `duplicateCheckLabel` 并存，`duplicateFlag = true` 对应 `duplicateCheckLabel IN (SAME, SIMILAR)`，两者写入逻辑保持同步
