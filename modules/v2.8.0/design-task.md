...
> **待提交说明 (v2.8.0 新增)**: 任务创建后初始状态为「待提交」（状态码 `-1`），创建人可在此状态删除任务，由创建人主动「提交」后才进入「待审核」流程。
>
...
| v2.8.0 | 待提交状态 | (v2.8.0 新增) 任务初始状态改为待提交（taskStatus = -1），由创建人主动提交后进入待审核；新增提交任务权限码（TJRW）；删除权限新增待提交状态 |
