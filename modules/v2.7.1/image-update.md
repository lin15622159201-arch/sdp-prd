...
| ✨ 图片更新任务 | 加急 (v2.7.1 新增) | boolean | 否 | false | 是否加急；创建时由设计师填写，默认不加急 |
...
| 字段名称 | 显示为 | 说明 |
|---------|--------|------|
| 任务信息 | 文本 + 标签 | 任务编号 + 任务类型（图片/视频）+ ✨ 加急标签（v2.7.1 新增，仅加急任务显示红色「加急」标签） |
...
| ✨ 加急 (v2.7.1 新增) | 开关 | 否 | 默认关闭；开启后任务列表显示「加急」标签 |
...
| v2.7.1 | 加急标签支持 | (v2.7.1 新增) 创建任务时支持设置加急（开关，默认关闭）；列表对加急任务展示红色「加急」标签；支持按加急筛选；数据模型新增 is_urgent 布尔字段 |
