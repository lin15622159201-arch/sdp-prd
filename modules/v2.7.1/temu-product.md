...
- module: style-management
  type: hard
  use_case: 款式推送上架后创建待上架记录，关联SPU/SKC数据；v2.7.1 起新增：展示样衣审版通过信息和款式难度（实时关联 prototype 表）
- module: spot-goods
  type: hard
...
| ✨ 删除 | `POP-SPGL-DSJ-SC` | (v2.8.0 新增) 删除待上架列表中的二次上架记录（仅 listing_type=2 且 release_status=待发布） |
...
| 实体 | 字段 | 状态值 | 说明 |
|------|------|--------|------|
| StyleOnShelves | review_status | 0=待审核, 1=已通过, 2=已驳回 | 内部审核状态 |
...
### ✨ 5.6 price_change_log（价格变动记录）(v2.7.1 新增)

| 属性 | 类型 | 必填 | 默认值 | 长度/范围 | 说明 |
|------|------|------|--------|----------|------|
| id | 整数 | 自动 | — | 雪花ID | 主键 |
| product_skc_id | 引用 | 自动 | — | — | 关联 product_skc 主键 |
| skc_code | 短文本 | 自动 | — | ≤32字符 | SKC编码，冗余存储 |
| platform | 短文本 | 自动 | — | ≤64字符 | 价格来源平台，取值来源：NEST字典 platform |
| price_before | 金额 | 自动 | — | DECIMAL(10,2) | 变动前价格；首次拉取时为 NULL |
| price_after | 金额 | 自动 | — | DECIMAL(10,2) | 变动后价格 |
| changed_at | 日期时间 | 自动 | — | — | 价格变动时间（定时任务检测到差异时的时间戳） |
| created_at | 日期时间 | 自动 | NOW() | — | 记录写入时间 |

**数据产生规则：**
- 定时任务每小时拉取 Temu 平台各 SKC 当前价格
- 与该 SKC 最新一条记录比对：价格不同则写入新记录，相同则跳过
- 首次拉取（无历史记录）：`price_before = NULL`，`price_after = 当前价格`
- 永久保留，不清理历史数据

### ✨ 5.7 product_skc 只读展示字段（v2.7.1 新增，来自 style-management）

以下字段为查询时实时关联 `prototype` 表返回，不冗余持久化到 product_skc。

| 展示字段名 | 来源表.字段 | 类型 | 说明 |
|-----------|-----------|------|------|
| plm_sample_approved_at | prototype.plm_sample_approved_at | 日期时间 | 样衣审版通过时间（只读） |
| plm_sample_approved_by | prototype.plm_sample_approved_by | 短文本 | 审版操作人（只读） |
| style_difficulty | prototype.style_difficulty | 枚举 | 款式难度：simple=简单 / normal=普通 / complex=复杂 |
...
| 审核 | `SH` | review_status=待审核 且 listing_type=1（首次上架） | 页面跳转 | 跳转到审核上架页面；(v2.4.0) 二次上架不支持审核驳回，无审核按钮 |
| 编辑 | `BJ` | review_status=已通过 且 release_status=待发布/发布失败 | 页面跳转 | 跳转到编辑待上架页面 |
| ✨ 删除 | `SC` | (v2.8.0 新增) listing_type=2（二次上架）且 release_status=0（待发布） | 二次确认弹窗 | 软删除该待上架记录；不回调源系统 |
...
1. 触发条件：listing_type=2（二次上架）且 release_status=0（待发布）
2. 操作方式：行操作「删除」→ 弹出二次确认弹窗，提示"确认删除该待上架记录？"
3. 确认后：对 style_on_shelves 记录执行软删除（`deleted=1`），不回调款式管理/现货管理
4. 不可删除场景：review_status≠待审核/已通过（如已发布/发布中），按钮不显示
...
- 审核通过：设置 review_status=已通过, release_status=待发布
- 审核驳回：设置 review_status=已驳回，填写驳回原因
  - 驳回时根据 source_type 分别回调款式管理或现货管理，将 SKC 的上架状态更新为"上架失败"
...
| 创建时间 | 日期范围选择 | 起止日期，不可选未来日期 |
| 款式标签 | 单选 | 取值来源：NEST字典 product_tag |
| ✨ 款式难度 (v2.7.1 新增) | 单选 | 全部 / 简单 / 普通 / 复杂；来自关联款式 style_difficulty 字段 |
...
| SKU | 数字 | SKU数量 |
| ✨ 款式难度 (v2.7.1 新增) | 文本 | 简单 / 普通 / 复杂；来自关联款式，数据为空时显示「—」 |
| ✨ 样衣审版 (v2.7.1 新增) | 格式化文本 | 审版通过时间 + 操作人；未审版时显示「—」；数据来自 plm_sample_approved_at / plm_sample_approved_by |
...
| ✨ 价格变动历史 (v2.7.1 新增) | — | SKC 行始终显示 | 侧弹窗 | 打开该 SKC 的价格变动历史 Drawer |
...
款式管理/现货管理（源系统）的字段发生变更后，下游的「待上架记录」（style_on_shelves）和「已发布商品」（product）需要按字段粒度执行不同的同步策略。本章定义各字段的同步策略，取代原来的全量快照方案。

**同步策略定义**：

| 策略 | 触发时机 | 说明 |
|------|---------|------|
| 直接修改 | 源系统字段变更后立即同步 | 下游记录静默更新，无需用户确认 |
| 确认后可修改 | 源系统字段变更后产生待同步标记 | 下游记录出现「待更新」标签，由运营人员手动确认后更新 |
| 修改后需通知 | 源系统字段变更后通知关联商品运营人员 | 发送系统通知/消息，不自动更新，由运营人员手动处理 |
| 不可修改 | — | 该字段在对应阶段已锁定，源系统变更不传播 |

---

### SPU 维度字段同步策略

| 字段 | 款式管理是否可修改 | 待上架更新方式 | 商品更新方式 | 备注 |
|------|---------------|-------------|------------|------|
| 品类 | 是 | 直接修改 | 确认后可修改 | |
| 款式标签 | 是 | 直接修改 | 确认后可修改 | |
| 款式级别 | 是 | 直接修改 | 直接修改 | |
...
| 织造方式 | 是 | 直接修改 | 修改后需通知 | |
| 季节 | 是 | 直接修改 | 修改后需通知 | |
| 尺码组 | 是 | 驳回后修改 | 不可修改 | 推送上架/PLM后不可修改 |
| 印花类型 | 是 | 直接修改 | 直接修改 | |
| 视觉形式 | 是 | 直接修改 | 直接修改 | |
| 版型 | 是 | 直接修改 | 直接修改 | |
| 风格 | 是 | 直接修改 | 直接修改 | |
...
| 制作方式 | 是 | 直接修改 | 直接修改 | |
| 前置拆版 | 是 | 直接修改 | 确认后可修改 | |
| 波段 | 是 | 直接修改 | 确认后可修改 | 已发布不可修改；未上架的确认后可修改，应由上架运营员通确认后修改 |
| 项目类型 | 是 | 直接修改 | 确认后可修改 | |
| 设计师 | 是 | 直接修改 | 修改后需通知 | |
| 设计组 | 是 | 直接修改 | 直接修改 | |
...
**直接修改**：
- 源系统字段写入后，后端异步将新值同步至 style_on_shelves / product 对应字段
- 同步失败记录日志，支持重试；失败不阻塞源系统修改操作

**确认后可修改**：
- 源系统字段变更后，在商品列表对应商品行追加「待更新」标签（`product_tag` 中写入 `待更新`）
- 运营人员在商品列表手动点击「更新字段」或进入编辑页面确认后，新值才写入 product 记录
- 待上架记录处于「待审核」状态时视同「直接修改」（尚未发布，可安全覆盖）

**修改后需通知**：
- 源系统字段变更后，向该商品的运营负责人发送系统消息，提示对应字段已变更
- 不自动更新 product 记录；运营人员决定是否手动在商品列表更新
- 通知内容：变更字段名、变更前后值、款式编码

**不可修改（待上架侧「驳回后修改」）**：
- 「驳回后修改」指字段只能在待上架记录被驳回重新编辑时修改，正常状态下锁定
- 对应商品侧「不可修改」：商品一旦发布，该字段不再接受源系统同步
...
**加入站点后自动测价通过 (v2.4.1 新增)**:
1. 操作入口：商品详情页编辑 → 设置/修改站点ID（site_id）
2. 触发逻辑：当 site_id 从空值变更为有值（即商品首次加入站点）时，系统自动触发以下操作：
   - 在 product_tag 数组中追加"测价通过"标签（如已存在则不重复添加）
   - 操作人记录为"系统"
   - 审计日志记录该自动操作
3. 用户提示：加入站点保存成功后，前端提示"商品已加入站点，系统已自动标记为测价通过"
4. 不触发场景：
   - site_id 未发生变化（编辑其他字段）
   - site_id 从一个站点变更为另一个站点（非首次加入）
   - site_id 被清空（从有值变为空）

---

## 11. ✨ 价格变动历史（侧弹窗）(v2.7.1 新增)

### 页面说明

**入口**：商品列表 → SKC 行 → 点击「价格变动历史」

**交互形式**：侧弹窗（Drawer），宽度建议 480px

**说明**：展示当前 SKC 在各平台的历史价格变动记录，只读，不可编辑。
...
|---------|--------|------|
| 平台 | 文本 | 价格来源平台名称 |
| 变动前价格 | 金额 | 首条记录显示「—」（首次拉取时无前值） |
| 变动后价格 | 金额 | — |
| 变动时间 | 日期时间 | 格式：YYYY-MM-DD HH:mm |

### 列表行为

- 按 `changed_at` 倒序排列
- 无分页，Drawer 内滚动展示全量记录
- 无筛选条件
...
| v2.8.0 | 二次上架删除 + 字段同步策略 | (v2.8.0 新增) 待上架列表支持删除二次上架待发布记录（软删除，不回调源系统）；新增字段同步策略章节，定义款式管理/现货管理→待上架/商品的 SPU/SKC 各字段同步方式（直接修改/确认后可修改/修改后需通知/不可修改） |
| v2.7.1 | 价格变动记录 + 样衣审版展示 + 款式难度查询 | (v2.7.1 新增) 新增 price_change_log 实体，由小时级定时任务写入平台价格变动，永久保留；商品列表 SKC 行新增「价格变动历史」侧弹窗；商品列表新增「款式难度」展示列和筛选项（简单/普通/复杂）；商品列表 SKC 行新增「样衣审版」只读字段（审版时间+操作人，来自关联款式 prototype 表，不持久化） |
