...
| project_type_code | 短文本 | 否 | — | ≤64字符 | 项目类型编码 (v2.4.1 新增) |
| project_type_name | 短文本 | 否 | — | ≤64字符 | 项目类型名称 (v2.4.1 新增) |
...
| project_type_code | 短文本 | 否 | — | ≤64字符 | 项目类型编码 (v2.4.1 新增) |
| project_type_name | 短文本 | 否 | — | ≤64字符 | 项目类型名称 (v2.4.1 新增) |
| develop_tags | JSON数组 | 否 | [] | 单个标签≤50字符，最多20个 | 开款标签列表，由 Agent 推送开款任务时写入，支持多个标签 (v2.6.2 新增) |
| ✨ plm_sample_approved_at | 日期时间 | 否 | NULL | — | 最新一次样衣审版通过时间（来自 PLM webhook，覆盖写）(v2.7.1 新增) |
| ✨ plm_sample_approved_by | 短文本 | 否 | NULL | ≤64字符 | 最新一次审版操作人姓名（来自 PLM webhook payload，覆盖写）(v2.7.1 新增) |
...
### 5.4 SKC与Agent任务关联（skc_agent_task 表）(v2.6.2 新增)
...
> 聚合状态为前端实时计算的展示态，不持久化。skc_agent_task 实体主定义在本模块；现货管理通过 SKC id 查询，不重复定义。

### ✨ 5.5 plm_sample_review_log（样衣审版日志）(v2.7.1 新增)

| 属性 | 类型 | 必填 | 默认值 | 长度/范围 | 说明 |
|------|------|------|--------|----------|------|
| id | 整数 | 自动 | — | 雪花ID | 主键 |
| prototype_id | 引用 | 是 | — | — | 关联 SKC 主键 |
| design_code | 短文本 | 是 | — | ≤32字符 | SKC 编码，冗余存储 |
| approved_at | 日期时间 | 是 | — | — | 本次审版通过时间（来自 PLM payload） |
| approved_by | 短文本 | 是 | — | ≤64字符 | 本次审版操作人 |
| plm_payload | 长文本 | 否 | — | — | PLM 原始 webhook payload（JSON，便于排查） |
| created_at | 日期时间 | 自动 | NOW() | — | 记录写入时间 |

**写入规则：**
- 每次收到 PLM 样衣审版通过 webhook，同时执行两步：
  1. 覆盖写 `prototype.plm_sample_approved_at` 和 `prototype.plm_sample_approved_by`（保留最新值）
  2. 向 `plm_sample_review_log` 追加一条日志（保留全部历史）
- 幂等键：`(prototype_id, approved_at)`，重复推送不重复写入日志

### ✨ 5.6 skc_store_material（SKC 店铺营销图）(v2.8.2 新增)
...
| store_name | 短文本 | 是 | — | ≤64字符 | 店铺名称，冗余存储 |
| marketing_images | JSON数组 | 是 | — | 1-10张 | 该店铺维度的营销图 URL 列表 |
| updated_at | 日期时间 | 自动 | NOW() | — | 最后更新时间 |
| updated_by | 短文本 | 是 | — | ≤64字符 | 最后更新操作人 |
| created_at | 日期时间 | 自动 | NOW() | — | 记录创建时间 |

**写入规则：**
- 唯一索引：`(prototype_id, store_id)`，同一 SKC 同一店铺只保留一条记录，写入时覆盖
- 由图片更新任务审核通过后触发写入（当任务设置了目标店铺时）
- 原 `prototype_material` 表保留，存储"通用营销图"（无店铺绑定），兼容存量数据
- 当任务未指定目标店铺时，仍写入 `prototype_material`（原有逻辑不变）
...
| 项目类型 | 多选 | 取值来源：NEST字典 project_type (v2.4.1 新增) |
...
| 智能编辑任务 | 多选 | 未发起/进行中/待审核/已完成/异常 (v2.6.2 新增) |
| 开款标签 | 多选 | Agent 推送的开款标签列表 (v2.6.2 新增) |
...
| 字段名称 | 显示为 | 说明 |
|---------|--------|------|
| SPU信息 | 可点击文本 + 文本 | SPU编码（可复制）+ 品类名 + SPU生成时间 |
| SKC信息 | 缩略图 + 可点击文本 + 文本 | 缩略图 + SKC编码（可复制）+ 颜色 + 尺码 + SKC生成时间 |
| 款式类型 | 标签 | 正常款/复色款 |
| 款式资料 | 状态标签 | 未提交/已提交 |
| 项目类型 | 文本 | 项目类型名称 (v2.4.1 新增) |
...
| 关联任务 | 聚合状态标签 + 悬浮列表 | (v2.6.2 新增) 展示 SKC 关联的 Agent 任务聚合状态；悬浮显示任务列表（任务编号/类型/状态），待审核任务置顶；含【审核】按钮（待审核时可操作，跳转对应任务审核界面）；任务编号可点击跳转任务中心并筛选对应任务 |
| 开款标签 | 标签组 | (v2.6.2 新增) 展示 Agent 推送的开款标签，多个标签并排展示 |
| ✨ 样衣审版 | 格式化文本 | (v2.7.1 新增) 审版通过时间 + 操作人；未审版时显示「—」；数据来自 plm_sample_approved_at / plm_sample_approved_by |
...
| 上传图片 | — | 弹窗 | 批量上传图片 |
| 导出 | `DC` | — | 导出列表数据 |
| 发送到 | `FSD` | 下拉菜单 | 图片更新任务/姿势裂变/虚拟换衣/智能开款(v2.6.2 新增) |
| 同步任务状态 | — | 工具栏按钮 | (v2.6.2 新增) 手动触发当前页所有 SKC 关联 Agent 任务状态刷新；弥补回调延迟 |
| 推送PLM | `TSPLM` | 弹窗 | 推送选中款式到PLM |
| 取消 | `QXSKC` | 弹窗 | 取消选中的SKC，填写取消原因 |
...
| sku_id | SKU编号 |
| name | SKC编号 |
| properties_value | 商品颜色+尺码，格式：`[颜色;尺码]`，如 `[蓝色;XXL]`（取商品属性，非款式属性） |
| labels | 店铺+项目类型 |
| 图片链接 | 待上架SKC图，仅第一张 |
| 重量/长/宽/高 | 商品维度输入值 |
...
- **路径1（已有逻辑）**：待上架商品发布通过后 → 推送该商品关联的全部SKU到聚水潭
- **路径2（v2.6.0 新增）**：某 SPU 下已存在已发布商品时，在款式管理或现货管理中新增SKC（含复色）→ 保存成功后**立即**推送该SKC关联的全部SKU到聚水潭，**不等待**上架审核，不受上架状态限制

> 路径2触发条件：`SPU下存在任一 product_status=1（已发布）的商品`。满足条件时，新SKC保存即触发推送，推送失败不阻断SKC创建流程，异步重试。

**路径2默认值规则**：新建SKC时，若商品维度尺寸未填写，推送聚水潭时使用以下默认值：
...
**开款标签推送 PLM (v2.6.2)**: 创建开款任务时，若关联 SKC 存在 `develop_tags`，则将标签列表全量推送至 PLM；单个 SKC 可有多个开款标签；推送失败不阻断任务创建流程，记录推送日志并支持重试。
...
| v2.6.0 | 聚水潭推送补全 | (v2.6.0 新增) SPU已有已发布商品时，款式管理/现货管理新增SKC（含复色）保存后立即推送SKU到聚水潭，不受上架状态限制；路径2推送使用默认尺寸（重量0.3kg/长40cm/宽30cm/高1cm） |
| v2.6.2 | Agent智能编辑接入 | (v2.6.2 新增) 「发送到」新增智能开款入口（≤50条，不含已取消）；列表新增智能编辑任务多选筛选、开款标签多选筛选、关联任务列（聚合状态+悬浮详情+审核跳转）、开款标签列；工具栏新增同步任务状态按钮；SKC数据模型新增 develop_tags 字段；新增 skc_agent_task 关联实体；新增 Agent 平台与 PLM 为 related_features |
...
| 标签页-基础信息 | 波段、营销图、设计图、视频、开发信息（颜色、尺码、是否拼接、制作方式、前置拆版、备注） | 可编辑 (v2.4.1 变更) 波段从SPU维度移至SKC维度；(v2.8.2 变更) 营销图展示调整为多组列表 |
| 标签页-修图任务 | 修图任务状态与详情 | — |
| 标签页-BOM物料 | BOM物料图片信息 | — |
| 底部操作栏 | 提交、取消 | 全局操作按钮 |
...
### 营销图多组展示规则（v2.8.2 新增，需求8）

SKC 详情页「基础信息」标签页中，营销图区域改为列表形式展示多组：

| 行 | 来源 | 说明 |
|----|------|------|
| 通用营销图（第一行，始终展示） | `prototype_material` 表 | 原有逻辑，兼容存量数据；无数据时显示空占位 |
| 各店铺营销图（动态行） | `skc_store_material` 表 | 按店铺名称排序；无该店铺数据则不展示 |

**交互规则**：
- 每行独立展示该组图片缩略图（最多10张）
- 每行支持独立上传/删除操作，权限与原营销图编辑权限一致
- 直接在 SKC 详情页编辑店铺营销图不经过图片更新任务流程（快速编辑入口）
...
| v2.6.2 | Agent智能编辑接入 | (v2.6.2 新增) 「发送到」新增智能开款；列表新增智能编辑任务筛选、开款标签筛选与展示、关联任务列、同步任务状态按钮；SKC新增 develop_tags 字段；新增 skc_agent_task 实体；创建开款任务时全量推送 SKC 开款标签至 PLM |
| v2.7.1 | PLM 样衣审版对接 | (v2.7.1 新增) 接收 PLM webhook 推送样衣审版通过事件；prototype 表新增 plm_sample_approved_at / plm_sample_approved_by 字段（覆盖写最新记录）；新增 plm_sample_review_log 日志表保留全部审版历史（幂等键：prototype_id + approved_at）；款式列表新增样衣审版只读展示列 |
| v2.8.2 | SKC 店铺维度营销图 | (v2.8.2 新增，需求8) 新增 skc_store_material 实体，存储 SKC 按店铺维度的营销图组（唯一索引：prototype_id + store_id）；保留 prototype_material 作为通用营销图兼容存量；SKC 详情页营销图区域改为多组列表展示（第一行通用图 + 各店铺行），支持独立上传/删除 |
