...
| project_type_code | 短文本 | 否 | — | ≤64字符 | 项目类型编码（v2.6.0 变更 #232: 项目类型改为SKC维度，此快照字段保留用于兼容） |
| project_type_name | 短文本 | 否 | — | ≤64字符 | 项目类型名称（v2.6.0 变更 #232: 项目类型改为SKC维度） |
...
| 属性 | 类型 | 必填 | 默认值 | 长度/范围 | 说明 |
|------|------|------|--------|----------|------|
| project_type_code (v2.6.0 新增) | 短文本 | 否 | — | ≤64字符 | 项目类型编码（SKC维度），取值来源：NEST字典，v2.6.0从SPU维度迁移至SKC维度 |
| project_type_name (v2.6.0 新增) | 短文本 | 否 | — | ≤64字符 | 项目类型名称 |
| skc_create_time (v2.6.0 新增) | 日期时间 | 否 | — | — | SKC创建时间 |

> (v2.4.1 变更) 波段从 SPU 维度改为 SKC 维度传递，每个 SKC 可独立设置波段。
> (v2.6.0 变更 #232) 项目类型从 SPU 维度改为 SKC 维度传递，每个 SKC 可独立设置项目类型。
...
| 字段名称 | 显示为 | 说明 |
|---------|--------|------|
| 款式信息 | 缩略图 + 可点击文本 + 标签 | SPU编码（可复制）+ 品类名 + 款式标签 (v2.6.0 变更 #232: 项目类型移至SKC维度) |
...
| SKC | 文本 | SKC编码 + 颜色 + 项目类型(v2.6.0 新增, SKC维度) + 波段(SKC维度，v2.4.1 变更) + SKC创建时间(v2.6.0 新增)（含"前置拆版"标记） |
...
| 字段名称 | 显示为 | 说明 |
|---------|--------|------|
| 款式信息 | 缩略图 + 可点击文本 + 标签 | SPU编码（可复制）+ 品类名 + 款式标签 + 平台SPU (v2.6.0 变更 #232: 项目类型移至SKC维度) |
| 运营信息 | 文本 | 店铺名 + 运营人员 |
| 款式类型 | 文本 | (v2.4.1 新增) 款式类型名称 |
| 商品标签 | 标签列表 | 动销款(蓝)/待更新(红)/已拆版(蓝)/测价通过(蓝) |
| SKC | 文本 | SKC编码 + 颜色 + 项目类型(v2.6.0 新增, SKC维度) + 波段(SKC维度，v2.4.1 变更) + SKC创建时间(v2.6.0 新增) + 平台SKC + 前置拆版标记 |
| SKU | 数字 | SKU数量 |
...
| v2.6.0 | #232 SKC维度改造：项目类型迁移 | (v2.6.0 变更 #232) 项目类型从SPU维度改为SKC维度：待上架列表SKC行独立展示项目类型、SPU行移除项目类型；商品列表SKC行独立展示项目类型、SPU行移除项目类型；(v2.6.0 新增) SKC创建时间字段：待上架列表SKC行新增、商品列表SKC行新增；(v2.6.0 变更) 数据模型：StyleOnShelves项目类型快照改为SKC维度(保留兼容)；(v2.6.0 新增) skc_on_shelves新增项目类型和SKC创建时间字段 |
