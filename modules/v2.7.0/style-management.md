...
- "Temu 平台（上架同步）"
- "AIGC 选款/灵感设计需求（款式来源）"
- "agent 平台（写入利润数据，用于自动测价通过判断）(v2.7.0 新增)"
- "飞书（测价通过/撤销时通知关联设计师）(v2.7.0 新增)"
---
...
| price_test_passed | 布尔 | 是 | false | — | (v2.7.0 新增) SKC维度测价通过标识，替代原商品标签中的测价通过逻辑 |
| price_test_passed_time | 日期时间 | 否 | — | — | (v2.7.0 新增) 测价通过时间 |
| price_test_passed_by | 短文本 | 否 | — | ≤64字符 | (v2.7.0 新增) 测价通过操作人（"系统"或操作人姓名） |
| profit_margin | 小数 | 否 | — | DECIMAL(10,4) | (v2.7.0 新增) 利润率，由 agent 平台通过接口写入；仅用于展示和自动测价通过判断，不阻断手动操作 |
| profit_updated_time | 日期时间 | 否 | — | — | (v2.7.0 新增) 利润数据最后更新时间，由 agent 平台写入时同步更新 |
...
| 推送PLM | `TSPLM` | 弹窗 | 推送选中款式到PLM（v2.7.0 变更：前置拆版款式仅允许在动销后自动推送或推送失败时手动重试，详见关键业务规则） |
| 取消 | `QXSKC` | 弹窗 | 取消选中的SKC，填写取消原因 |
| 从PLM同步款式 | `SYNPLM` | 弹窗 | (v2.7.0 新增) 批量输入PLM款号，将PLM数据导入款式管理列表 |
| 测价通过 | `CJ` | 操作确认 | (v2.7.0 变更) 手动标记选中SKC为测价通过，无论是否有利润数据均可操作 |
| 撤销测价通过 | `CJ` | 操作确认 | (v2.7.0 新增) 仅 price_test_passed=true 且 disassembly_finished=0 时可操作；撤销后飞书通知关联设计师 |
| 推送上架 | `TSSJ` | 弹窗 | 选择发布店铺，推送到待上架列表 |
| 复色 | `FS` | — | 选中1条SKC，生成一条新的复色SKC |
| 打印版单 | `DYBD` | — | 批量打印版单 |
| 查看详情 | `CKXQ` | 新标签页 | (v2.7.0 变更) 跳转到SKC详情页，由当前页跳转改为新标签页打开，保留列表页状态 |
...
**测价通过 (v2.7.0 重构)**: 改为 SKC 维度独立标识，读取 `prototype.price_test_passed` 字段，不再依赖商品标签。触发路径：
- **自动**：agent 平台通过接口写入利润数据（`profit_margin`）后，系统判断利润是否达标，自动将 `price_test_passed` 置为 true，操作人记为"系统"。
- **手动**：运营在款式列表操作「测价通过」，无论是否存在利润数据均可执行。

**撤销测价通过 (v2.7.0 新增)**: 可操作条件：`price_test_passed=true` 且 `disassembly_finished=0`（未完成拆版）。撤销后 `price_test_passed` 置为 false，系统触发飞书通知关联设计师，通知内容包含款号及撤销原因。测价通过时同样触发飞书通知关联设计师。

**推送PLM前置拆版校验 (v2.7.0 新增)**:
手动推送PLM时，对满足以下条件的SKC拦截操作：`pre_disassembly_state=1`（前置拆版=是）且 `push_plm_status=0`（待推送）。
- 拦截提示："前置拆版款式不支持手动推送PLM，动销后系统将自动推送"
- 例外：`push_plm_status=3`（推送失败）的前置拆版款式允许手动重试

**编辑SPU时SKC保存确认 (v2.7.0 新增)**: 点击「编辑SPU」按钮前，系统检测当前 SKC 详情页是否有未保存改动，如有则弹出确认弹窗：
- 【保存后继续】：先保存 SKC 数据，保存成功后打开编辑SPU弹窗
- 【直接打开（不保存）】：放弃 SKC 未保存内容，直接打开编辑SPU弹窗
...
| 是否拼接 | 单选按钮 | 否 | — | 是/否 |
| 制作方式 | 单选按钮 | 否 | — | 实物样(1)/3D样(2)；(v2.7.0 变更) 当项目类型=无版无样时，系统自动设为3D样，用户仍可手动修改 |
| 前置拆版 | 单选按钮 | 否 | — | 是(1)/否(0) |
| 车缝备注 | 多行文本 | 否 | ≤200字符 | |
| 裁剪工艺备注 | 多行文本 | 否 | ≤200字符 | |
| 版型备注 | 多行文本 | 否 | ≤200字符 | |
...
**制作方式联动 (v2.7.0 新增)**: 当项目类型（`project_type_code`）对应 NEST 字典「无版无样」时，`make_clothes_type` 自动设为 `3D样`（字典值 `makestyle=3D`，字段值 `2`），页面以浅色提示文案告知用户已自动填充。用户可手动修改为其他选项。
...
| v2.7.0 | 测价通过SKC维度重构+推送PLM校验+字段联动+交互优化+PLM同步入口 | 来自规划文档 规划文档_v2.7.0_SDP商品开发迭代.md：(v2.7.0 新增) SKC维度测价通过字段(price_test_passed/price_test_passed_time/price_test_passed_by/profit_margin/profit_updated_time)；撤销测价通过操作；从PLM同步款式弹窗；项目类型→制作方式字段联动；(v2.7.0 变更) 推送PLM增加前置拆版校验；款号点击改为新标签页；编辑SPU新增SKC保存确认弹窗；测价通过来源从商品标签改为SKC独立字段 |

---

## 8. 从PLM同步款式 (v2.7.0 新增)

### 页面说明

**入口**: 款式列表 → 点击「从PLM同步款式」按钮（需 `SYNPLM` 权限）

**交互形式**: 弹窗

### 操作说明

输入模式支持切换：SPU号 / SKC号，文本框支持批量粘贴，多个编号以逗号、空格或换行分隔。

### 校验规则

系统对每个输入编号依次执行两层校验：

**校验1 — 款式是否已存在**：查询款式管理列表中是否已有该款号。若已存在，标记为"款式已存在"错误，该条目不继续校验2，但不阻断其余条目。

**校验2 — PLM中是否存在**：通过PLM接口查询款式信息。若PLM中不存在，标记为"PLM中未找到该款式"错误。

校验通过的条目：从PLM获取完整款式数据，添加到款式管理列表，任务来源标记为 `task_source=plm_sync`。

### 提交结果

弹窗底部展示执行明细：成功 N 条、失败 N 条，失败条目逐行显示编号与失败原因，支持复制失败列表重新处理。
...
|---------|-------------|
| 款式已存在 | 该行标红，提示"款式已存在" |
| PLM中不存在 | 该行标红，提示"PLM中未找到该款式" |
| PLM接口超时 | 提示"PLM接口请求超时，请稍后重试" |
| 输入框为空提交 | 提示"请输入至少一个款号" |
