...
- 每张卡片显示：
  ...
  - 设计师、波段、店铺、平台、价格信息
  - 同款/相似款入口按钮（显示数量，无数据时显示"暂无同款"）；(v2.7.0 变更) 同款/相似款卡片新增尺码组字段，区分标码与大码
  - 操作区：通过 / 淘汰 / 暂不处理 + 备注
- 查看大图：点击图片进入全屏浏览，可切换查看当前开款任务的所有图片
- **(v2.7.0 新增) 顶部筛选区**：支持按以下条件实时过滤卡片（本地过滤，不触发后端请求）：
  - 有同款（`sameStyleNum > 0`）
  - 有相似款（`similarStylesNum > 0`）
...
| 批量同意 (v2.7.0 新增) | 勾选多张卡片后点击「批量同意」，批量设置 checkResult = 'PASS' | 是（合并提交） |
| 批量淘汰 (v2.7.0 新增) | 勾选多张卡片后点击「批量淘汰」，批量设置 checkResult = 'DISUSE' | 是（合并提交） |

批量操作提交前弹出确认弹窗，显示"即将对 N 条数据执行{同意/淘汰}，确认继续？"。
...
| 项目类型 (v2.7.0 新增) | 无版无样 | 制作方式 | 自动设为「3D样」 | 根据 NEST 字典 makestyle=3D；用户可手动修改 |
...
| v2.7.0 | 审款批量操作+查重尺码组+批量开款字段联动 | 来自规划文档 规划文档_v2.7.0_SDP商品开发迭代.md：(v2.7.0 新增) 审款页面顶部筛选区（有同款/有相似款/款式标签）；批量同意/批量淘汰操作；查重卡片新增尺码组字段；批量开款字段联动规则新增项目类型=无版无样时自动设制作方式为3D样 |
