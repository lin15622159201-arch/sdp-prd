...
| 发送到智能开款 | 弹窗 | 现货管理-列表 → 「发送到」→「智能开款」 | 弹窗 | (v2.6.2 新增) 弹窗交互规格同款式管理，复用相同弹窗规格；提交后保存 skc_id + agent任务ID + 任务渠道 |
...
| 智能编辑任务 | 下拉多选 | 未发起/进行中/待审核/已完成/异常；按 SKC 维度聚合计算 | — | (v2.6.2 新增) |
...
| 关联任务 | 聚合状态标签 + 悬浮列表 | (v2.6.2 新增) SKC 关联 Agent 任务聚合状态；悬浮显示任务列表（任务编号/类型/状态），待审核置顶；含【审核】跳转按钮，任务编号可点击跳转任务中心 | |
...
| 发送到 | 始终显示 | 商品运营 | 下拉菜单 | (v2.6.2 新增) 图片更新/姿势裂变/虚拟换衣/智能开款；智能开款前置条件：选中 ≤50 条且不含已取消状态 |
| 同步任务状态 | 始终显示 | 商品运营 | 工具栏按钮 | (v2.6.2 新增) 手动触发当前页所有 SKC 关联 Agent 任务状态刷新 |
...
1. **回显SKC完整上下文**：页面打开时自动加载当前 SKC 的颜色、尺码、波段、商品图、上架状态、推送买手状态、测价状态、测价通过状态、测价通过时间、平台供货价，以及所属 SPU 摘要信息
...
| v2.6.0 | 聚水潭推送补全 | (v2.6.0 新增) SPU已有已发布商品时，现货管理新增SKC（创建SKC信息步骤保存）及复色保存后立即推送SKU到聚水潭，不受上架状态限制；推送失败不阻断创建流程，异步重试；未填写尺寸时使用默认值（重量0.3kg/长40cm/宽30cm/高1cm） |
| v2.6.2 | Agent智能编辑接入 | (v2.6.2 新增) 「发送到」新增智能开款入口（≤50条，不含已取消）；列表新增智能编辑任务多选筛选、关联任务列（聚合状态+悬浮详情+审核跳转）；工具栏新增同步任务状态按钮；共享 style-management 的 skc_agent_task 数据层；新增 Agent 平台为 related_feature |
