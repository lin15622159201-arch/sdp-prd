...
将所有参考文件拼接生成：
1. dist/build-prd-universal-prompt.md  — 独立可用的完整 prompt，适用于任意 LLM
2. dist/build-prd.skill               — 编译后的技能包
"""
...
ORDERED_FILES = [
    SKILL_DIR / "SKILL.md",
    REFS_DIR / "appendices" / "build-prd-appendix-typing.md",
    REFS_DIR / "规划文档结构.md",
    REFS_DIR / "文档结构.md",
    REFS_DIR / "通用设计语言规范.md",
    REFS_DIR / "文档目录.md",
    REFS_DIR / "appendices" / "build-prd-appendix-selfcheck.md",
]
...
def build_universal_prompt():
    ...
    parts = []
    parts.append("# Build-PRD 完整独立 Prompt\n")
    parts.append("> 本文件是 build-prd 技能的完整独立版本，可在任何 LLM 中直接使用。\n")
    parts.append("> 将本文件内容粘贴到 ChatGPT / Gemini / DeepSeek / Claude 等 LLM 中，")
    parts.append("> 然后提供你的业务需求和现有模块文档，即可生成规划文档并执行模块更新。\n")
    parts.append("---\n")
    ...
    for fpath in ORDERED_FILES:
        ...
        section_name = fpath.relative_to(SKILL_DIR).as_posix()
        parts.append(f"\n{'='*60}\n")
    ...
    out_path = DIST_DIR / "build-prd-universal-prompt.md"
    ...
    # 同时生成 .skill 文件
    skill_path = DIST_DIR / "build-prd.skill"
