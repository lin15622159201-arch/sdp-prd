...
const { workspace, dragging, getCanvas } = useEditorStore();
...
// 点击抓手, 切换抓手模式
const handleDrag = () => {
  dragging.dragMode = !dragging.dragMode;
};

watch(
  () => dragging.dragMode,
  (value) => {
    const isUsingPencil = toolbarStore.activeTool === TOOL_ENUM.BRUSH || toolbarStore.activeTool === TOOL_ENUM.ERASER;
    if (isUsingPencil) {
