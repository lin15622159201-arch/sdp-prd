...
import { IPickingStyleImportRes } from '../../../api/type';

export type HandleOpenDialog = (data: IPickingStyleImportRes) => void;

export const useImportResultDialog = () => {
  const data = ref({} as IPickingStyleImportRes);

  const summary = computed(() => `外部数据导入成功 ${data.value.successCount} 条，失败 ${data.value.failCount} 条 数据！`);
