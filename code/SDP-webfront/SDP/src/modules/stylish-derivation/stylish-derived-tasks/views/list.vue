...
import { styleGenPageApi } from '../api/index';
...
const add = () => {
  router.push({
    path: '/stylish-derivation/stylish-derived-tasks/create'
  });
};

// 中止任务
const batchAbort = () => {
  handleBatchAbort(selectionList.value.map((v: StyleGenPageResListItem) => v.taskId), handleBtnSearch);
};

// 删除任务
const batchDelete = () => {
  handleBatchDelete(selectionList.value.map((v: StyleGenPageResListItem) => v.taskId), handleBtnSearch);
};

// 重试任务
const batchRetry = () => {
  handleBatchRetry(selectionList.value.map((v: StyleGenPageResListItem) => v.taskId), handleBtnSearch);
};
