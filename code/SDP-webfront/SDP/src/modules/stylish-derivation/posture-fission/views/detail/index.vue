  ...
  <sc-app-page class="custom-app-page bg-gray">
    <template #fheader>
      <div class="fheader-box">
        <el-icon style="cursor: pointer;" @click="router.replace('/posture-fission/posture-fission/list')"><ArrowLeftBold /></el-icon>
        ...
        >删除</el-button>
        <el-button
          v-if="FZ && chooseProductList.taskStatus === 'COMPLETED'"
          @click="taskCopy"
          :loading="loading"
          type="primary"
        >复制任务</el-button>
      ...
      <div class="tw-h-100% main-box">
        ...
        <div class="main-left">
          ...
          <div class="text-left">
            ...
            <!-- <div class="m-5">
              是否裁头：{{ chooseProductList.cuttingHead === 1 ? '是' : (chooseProductList.cuttingHead === 0 ? '否' : '-') }}
            </div> -->
            <div class="m-5">
              服装类型：{{ chooseProductList.clothTypeName || '-' }}
            </div>
            <div class="m-5">
              构图跟随原图：{{ chooseProductList.layoutByRef === 1 ? '是' : (chooseProductList.layoutByRef === 0 ? '否' : '-') }}
            </div>
            <div class="m-5">
              是否需要背面：{{ chooseProductList.needBackSide === 1 ? '是' : (chooseProductList.needBackSide === 0 ? '否' : '-') }}
            </div>
            <div class="m-5">
              背面数量：{{ chooseProductList.backSideCount || '-' }}
            </div>
            <div class="m-5">
              生成数量：{{ chooseProductList.genCount || '-' }}
            </div>
            <div class="m-5">
              是否补全全身：{{ chooseProductList.completeBody === 1 ? '是' : (chooseProductList.completeBody === 0 ? '否' : '-') }}
            </div>
            <div class="m-5">
              分辨率：{{ poseChangeratio.find(v => v.value === chooseProductList.imgSize)?.label ?? '-' }}
            </div>
          </div>
        </div>
        <div class="main-right">
          ...
          <div class="titlt-left" v-if="dialogImgVisible">
            <div class="title-t" style="font-weight: 100;">
              ...
              <div style="margin-left: 10px;">已选中 {{ (chooseProductList.generateImages || []).filter(v => v.select).length }} 张</div>
            </div>
            <div
            ...
            >
              ...
              <TaskCollection
                ...
                :imgs="(chooseProductList.generateImages || []).filter(v => v.select).map(v => v.pictureId).join(',')"
              />
              <el-button
                :disabled="!(chooseProductList.generateImages || []).filter(v => v.select).length"
...
import { webStyleGenApi, userEvaluateImageGroupGetByBusIdApi, userEvaluateImageGroupSaveOrUpdateApi, imageDownloadLog } from '../../api/index';
import { ByIdRes, UserEvaluateImageGroupSaveOrUpdateReq, UserEvaluateImageGroupGetByBusIdRes, IImageDownloadLogReq } from '../../api/types';
import {
...
const { poseChangeratio } = useSearch();
...
const chooseProductList = ref<ByIdRes>({});
...
const handleDelete = async () => {
  await handleBatchDelete([chooseProductList.value.taskCode as string], goToBack);
};
...
watch(
  ...
  () => {
    if ((chooseProductList.value.generateImages || []).filter(v => !v.select).length) {
