...
import { TABS_LIST, TABS_ENUM } from '../list/constant';
import { useTimerangeDistance } from '@/hooks-transfer/use-timerange-distance';

interface IParams {
  handleEdit: (row: IDimensionPageResListItem, behavior: string) => void;
...
export const useListColumns = ({ handleEdit, viewProcessOrder, reloadFn, handleOperateLog }: IParams) => {
  const { WBBJ, WBXQ } = usePermissionConfig();
    ...
    return [
      ...
      },
      {
        ...
        render: (row) => {
          return (
            <>
              {row.patternFileList?.length && (
                row.patternFileList.map((it, ind) => {
                  if (row.patternFileList.length >= 1 && ind === 0) {
                    return (
                      <el-link
                        underline={false}
                        onClick={() => {
                          handleDownLoadFile(it.patternUrl, it.patternName);
                        }}
                      >
                        {it.patternVersion}
                      </el-link>
                  ...
                  return (<p class='tw-line-through'>{it.patternVersion}</p>);
                })
              )}
            </>
          );
            ...
            <>
              {WBXQ.value ? (
                <el-button
                  ...
                  text
                >
                  {filters.getEnumLabel(TABS_LIST, row.dimensionState!)}
                  {row.dimensionVersion ? `-${row.dimensionVersion}` : ''}
                </el-button>
              ) : (
                <>
                  {filters.getEnumLabel(TABS_LIST, row.dimensionState!)}
      ...
      //         {(row.dimensionState === TABS_ENUM.WAIT_TAKE
      //         || row.dimensionState === TABS_ENUM.WAIT_SUBMIT) && (
      //           <span
      ...
      //         {row.dimensionState === TABS_ENUM.SUBMIT && (
      //           <span
      //             v-html={handleCostTime({
      //               row,
      ...
      },
      {
        ...
        minWidth: '120',
        render(row) {
          return filters.formatTime(row.dimensionFinishTime);
        },
      },
      {
        label: '操作记录',
        ...
        render(row) {
          return (
            <>
              ...
              />
              <div>
                {(WBBJ.value && row.isLatest === YES_NO_ENUM.YES && row.dimensionState === '2') && (
                  <el-button
                    type='primary'
