...
import {
  SUBMIT_STATUS_LIST, QC_CRAFT_LIST, REMARK_BIZ_TYPE_ENUMS,
...
export const useListColumns = ({ handleEdit, viewProcessOrder, reloadFn, handleOperateLog }: IParams) => {
  const { NBBJ, NBXQ } = usePermissionConfig();
  ...
  const { columns } = useTableColumns<IDimensionPageResListItem>(() => {
    return [
            ...
            <>
              {NBXQ.value ? (
                <el-button
                  ...
                  text
                >
                  {filters.getEnumLabel(SUBMIT_STATUS_LIST, row.dimensionState!)}
      ...
      //         {row.dimensionState === SAMPLE_DEVELOP_STEP_STATE_ENMU.WORKING && (
      //           <span
      ...
      //         {row.dimensionState === SAMPLE_DEVELOP_STEP_STATE_ENMU.FINISHED && (
      //           <span
      //             v-html={handleCostTime({
      //               row,
