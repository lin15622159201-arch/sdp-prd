      ...
      switch (v.value) {
        case TABS_ENUM.WAIT: {
          const row = stateInfo.value
            .find(it => it.processNodeCode === PROCESS_NODE_CODE_ENUM.SEW_OUTER_PROCESS
              && it.nodeStateCode === '0');
          if (row) {
        ...
        case TABS_ENUM.SEMI: {
          const arr = stateInfo.value
            .filter(it => it.processNodeCode === PROCESS_NODE_CODE_ENUM.SEW_OUTER_PROCESS_HALF_SECOND_CRAFT
              && ['1', '2'].includes(it.nodeStateCode!));
          if (arr.length) {
            count = String(plus(...arr.map(it => it.unFinishCount!)));
          }
          break;
        }
        default:
