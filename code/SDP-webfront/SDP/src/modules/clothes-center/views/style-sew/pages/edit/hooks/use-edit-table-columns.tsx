...
import {
  ...
  ISampleQcBaseInfoResSizeInfoListItem,
} from '@/modules/clothes-center/views/style-sew/api/types';

interface IProps {
  detailData: Ref<DetailRes>;
  sizeInfoList: Ref<DetailRes['sizeInfoList']>;
  isView: Ref<boolean>;
}

export const useListColumns = ({ detailData, sizeInfoList, isView }: IProps) => {
  const clos = computed(() => {
    const firstList = sizeInfoList.value[0]?.clothesTrimSizeList || [];
    if (firstList.length) {
  ...
  const { columns } = useTableColumns<ISampleQcBaseInfoResSizeInfoListItem>(() => {
    return [
      {
        label: '部位',
        minWidth: '150',
      ...
      ...clos.value.map<ITableColumnsItem<ISampleQcBaseInfoResSizeInfoListItem>>((item: any) => ({
        label: item.label,
        align: 'center',
        minWidth: '120',
        render(row: DetailRes['sizeInfoList'][0]) {
          return (
            <input-number
