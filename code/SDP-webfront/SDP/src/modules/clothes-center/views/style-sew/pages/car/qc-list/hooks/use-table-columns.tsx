import { useTableColumns } from '@toy/business-components';
import { resizeImgByWidth } from '@/core/utils/helper';
import {
  QC_CRAFT_LIST, REMARK_BIZ_TYPE_ENUMS,
  SAMPLE_TYPE_LIST, MAKE_CLOTHES_TYPE_LIST,
  QC_STATUS_LIST, QC_RES_LIST,
  QC_STATUS_ENMU
} from '@/modules/clothes-center/constant';
import { filters } from '@/core/plugins/filter';
import {
  ISampleQcPageResListItem, ISampleQcPageResListItemCraftListItem
} from '@/modules/clothes-center/views/style-sew/api/types';
import { remarkAdd } from '@/modules/clothes-center/api';
import { useOpenBlank } from '@/hooks-transfer/use-router-blank';
import { useRouter } from 'vue-router';
import { YES_NO_ENUM } from '@/constant/global';
import { usePermissionConfig } from '@/modules/clothes-center/views/style-sew/use-permission-config';
import { ElMessage } from 'element-plus';
import { useTimerangeDistance } from '@/hooks-transfer/use-timerange-distance';

interface IProps {
  reloadFn: () => void;
  viewProcessOrder: (row: ISampleQcPageResListItem) => void;
  handleOperateLog: (clothesId: string) => void;
}

export const useListColumns = ({ reloadFn, viewProcessOrder, handleOperateLog }: IProps) => {
  const $router = useRouter();
  const { YYZJ_ZJ, YYZJ_XQ } = usePermissionConfig();
  const { handleCostTime } = useTimerangeDistance();

  // 添加备注
  const handleCreateRecord = async (row: any, remark: string) => {
    const remarkParams = {
      bizId: row.clothesId as string,
      bizType: REMARK_BIZ_TYPE_ENUMS.SAMPLE_CLOTHES,
      remark,
    };
    await remarkAdd(remarkParams);
    await reloadFn();
  };
  const handleEdit = (row: ISampleQcPageResListItem, behavior: string) => {
    if (row.isAbnormal === YES_NO_ENUM.YES && behavior === 'modify') {
      ElMessage.error('存在未处理异常的任务');
      return;
    }
    $router.push(
      {
        name: 'ClothesCenterStyleSewEdit',
        params: {
          id: row?.sampleQcId,
          behavior
        }
      }
    );
  };
  const { columns } = useTableColumns<ISampleQcPageResListItem>(() => {
    return [
      {
        type: 'selection',
        width: 50,
        selectable: row => row.isCancel === YES_NO_ENUM.NO
      },
      {
        label: '加工单号',
        minWidth: '150',
        render: (row) => {
          return (
            <>
              <el-button type='primary' link onClick={() => viewProcessOrder(row)}>
                {row.processCode}
              </el-button>
              <p>
                {row.isAbnormal === YES_NO_ENUM.YES && (<el-tag type='danger' class='tw-mr[4px]'>异常</el-tag>)}
                {row.isRepair === YES_NO_ENUM.YES && (<el-tag type='danger'>返修</el-tag>)}
                {row.isCancel === YES_NO_ENUM.YES && (
                  <el-tag type='danger'>
                    取消
                  </el-tag>
                )}
              </p>
            </>
          );
        }
      },
      {
        label: '设计款号',
        minWidth: '180',
        render: (row) => {
          return (
            <div class='tw-text-left'>
              <div class='tw-flex'>
                SKC：
                <sc-copy-text text={row.designCode} />
              </div>
              <div class='tw-flex'>
                SPU：
                <sc-copy-text text={row.styleCode} />
              </div>
            </div>
          );
        }
      },
      {
        label: '图片',
        align: 'center',
        minWidth: '120',
        render(row) {
          const spuShelvePictureList = (row.shelvePicture?.spuShelvePictureList || []);
          const skcShelvePictureList = (row.shelvePicture?.skcShelvePictureList || []);
          const images = [...skcShelvePictureList, ...spuShelvePictureList, ...(row.designPictureList || [])];
          return (
            <custom-image
              src={resizeImgByWidth(images?.[0] || '', 192)}
              class='img-thumbnail__table'
              fit='cover'
              preview-src-list={images}
              preview-teleported
            />
          );
        },
      },
      {
        label: '打版信息',
        width: '120',
        render: (row) => {
          return (
            <>
              <p>{filters.getEnumLabel(SAMPLE_TYPE_LIST, row.sampleType!)}</p>
              <p>{filters.getEnumLabel(MAKE_CLOTHES_TYPE_LIST, row.makeClothesType!)}</p>
            </>
          );
        }
      },
      {
        label: '车版件数',
        width: '120',
        prop: 'sampleAmount',
      },
      {
        label: '相关人员',
        minWidth: '140',
        render(row) {
          return (
            <>
              <div>
                设计师：
                { row.designerName || '-' }
              </div>
              <div>
                车缝师：
                { row.sewerName || '-' }
              </div>
              <div>
                纸样师：
                { row.patternMakerName || '-' }
              </div>
              <div>
                质检师：
                { row.qualityCheckerName || '-' }
              </div>
              <div>
                审版工艺师：
                {row.reviewCraftsmanName || '-'}
              </div>
            </>
          );
        }
      },
      {
        label: '任务状态',
        width: '120',
        render: (row) => {
          return (
            <>
              {YYZJ_XQ.value ? (
                <el-button
                  onClick={() => handleEdit(row, 'view')}
                  type='primary'
                  text
                >
                  {filters.getEnumLabel(QC_STATUS_LIST, row.qcStatus!)}
                  {row.auditVersionNum ? `-${row.auditVersionNum}` : ''}
                </el-button>
              ) : (
                <>
                  {filters.getEnumLabel(QC_STATUS_LIST, row.qcStatus!)}
                  {row.auditVersionNum ? `-${row.auditVersionNum}` : ''}
                </>
              )}
            </>

          );
        }
      },
      {
        label: '质检结果',
        width: '120',
        type: 'enum',
        prop: 'qcResult',
        options: QC_RES_LIST,
      },
      {
        label: '二次工艺&裁剪方式',
        minWidth: '150',
        render(row) {
          return (
            <>
              <div>
                {row.craftList.map((item: ISampleQcPageResListItemCraftListItem) => {
                  return (
                    <div>
                      { item.craftsProcessName || filters.getEnumLabel(QC_CRAFT_LIST, item.craftsRequire!)}
                      ：
                      {(item.nameList || []).map((name: string) => (
                        <span>
                          {name || '-'}
                          ；
                        </span>
                      ))}
                    </div>
                  );
                })}
              </div>
              {row.cuttingMethod && (
                <el-tag>
                  {row.cuttingMethod}
                </el-tag>
              )}
            </>
          );
        }
      },
      // {
      //   label: '耗时',
      //   minWidth: '120',
      //   render: (row) => {
      //     // 需要 row 、当前时间、创建时间
      //     return (
      //       <>
      //         {/* 待质检耗时=当前时间-版本数据待质检创建时间-异常时间 */}
      //         {row.qcStatus === QC_STATUS_ENMU.PENDING && (
      //           <span
      //             v-html={handleCostTime({
      //               row,
      //               currentTimeKey: '',
      //               stepCreatedTimeKey: 'qcCreatedTime',
      //               stepTimeConsuming: 'stepExceptionTimeConsuming',
      //               hasMinus: true,
      //               isBeforeStageTime: true
      //             })}
      //           />
      //         )}
      //         {/* 已质检耗时=提交质检时间-版本数据待质检创建时间-异常时间 */}
      //         {row.qcStatus === QC_STATUS_ENMU.OVER && (
      //           <span
      //             v-html={handleCostTime({
      //               row,
      //               currentTimeKey: 'qcTime',
      //               stepCreatedTimeKey: 'qcCreatedTime',
      //               stepTimeConsuming: 'stepExceptionTimeConsuming',
      //               hasMinus: true,
      //               isBeforeStageTime: true
      //             })}
      //           />
      //         )}
      //       </>
      //     );
      //   }
      // },
      {
        label: '创建时间',
        minWidth: '120',
        render(row) {
          return filters.formatTime(row.qcCreatedTime);
        },
      },
      {
        label: '提交时间',
        minWidth: '120',
        render(row) {
          return filters.formatTime(row.qcTime);
        },
      },
      {
        label: '操作记录',
        width: '180px',
        fixed: 'right',
        render(row) {
          return (
            <>
              <remark-record
                v-model={row.remark}
                name-key='createdName'
                time-key='createdTime'
                desc-key='remark'
                onCreate={(remark: string) => handleCreateRecord(row, remark)}
                v-slots={{
                  append: () => (
                    <el-button
                      type='primary'
                      text
                      onClick={() => handleOperateLog(row.clothesId as string)}
                    >
                      操作日志
                    </el-button>
                  )
                }}
              />
              <div>
                {/* 待质检才显示 */}
                {YYZJ_ZJ.value && row.qcStatus === '0' && (
                  <el-button
                    type='primary'
                    onClick={() => handleEdit(row, 'modify')}
                    disabled={row.isCancel === YES_NO_ENUM.YES}
                  >
                    质检
                  </el-button>
                )}
              </div>
            </>
          );
        }
      },
    ];
  });
  return {
    tableColumns: columns
  };
};
