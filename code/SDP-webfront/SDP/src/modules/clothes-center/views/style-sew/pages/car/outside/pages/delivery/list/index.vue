<template>
  <sc-app-page :laside="laside">
    ...
    <template #header>
      <div class="tw-flex tw-mb[10px]" v-if="activeTab === TABS_ENUM.WAIT">
        <el-button
          type="danger"
        ...
        <el-button
          type="primary"
          v-if="params.clothesStepNodeState === SAMPLE_DEVELOP_STEP_STATE_ENMU.WORKING && WBCF_QRSH"
    ...
    <repair-receipt
      ...
      :sampleAmount="selectedList?.[0]?.sampleAmount"
...
import { ref, computed, watch, reactive, toRefs, PropType, onMounted } from 'vue';
...
import { sewQueryByPage, sewReceipt } from '@/modules/clothes-center/views/style-sew/api';
import {
...
import { ISewQueryByPageResListItem, ISewReceiptReq } from '@/modules/clothes-center/views/style-sew/api/types';
...
import RepairReceipt from './component/repair-receipt.vue';
...
const { YCFQ, WBCF_QRSH } = usePermissionConfig();
...
const { searchConfig } = useSearch({ mode: 'delivery' });
...
const handleParamsData = (customParams: any) => {
  ...
  rest.sewReceiptFinishTimeStart = rest.finishTimeStart || '';
  rest.sewReceiptFinishTimeEnd = rest.finishTimeEnd || '';
  rest.sewReceiptCreatedTimeStart = rest.createdTimeStart || '';
  rest.sewReceiptCreatedTimeEnd = rest.createdTimeEnd || '';
  delete rest.timeConsumingType;
  delete rest.timeConsumingStart;
  delete rest.timeConsumingEnd;
  delete rest.finishTimeStart;
  delete rest.finishTimeEnd;
  delete rest.createdTimeStart;
  delete rest.createdTimeEnd;
  return {
    ...rest,
    ...row,
    ...timeParams
...
const {
  ...
  request: {
    ...
    params: {
      ...
      sewerIdList: [],
      cutterIdList: [],
      sewReceiptCreatedTimeStart: '',
      sewReceiptCreatedTimeEnd: '',
      sewReceiptFinishTimeStart: '',
      sewReceiptFinishTimeEnd: '',
      finishTimeStart: '',
      finishTimeEnd: '',
      createdTimeStart: '',
      createdTimeEnd: '',
...
const handleRepairReceipt = async (receivedNum: string = '') => {
  const param: ISewReceiptReq = {
    sewReceiptId: selectedList.value[0].sewReceiptId || '',
    pieces: receivedNum,
  };
  await sewReceipt(param);
