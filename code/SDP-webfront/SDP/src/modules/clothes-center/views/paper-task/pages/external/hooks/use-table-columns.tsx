...
export const useListColumns = ({ handleEdit, viewProcessOrder, reloadFn, handleOperateLog, activeTab }: IParams) => {
  ...
  const { columns } = useTableColumns<IPatternClothesQueryByPageResListItem>(() => {
    return [
      ...
      //         {(row.processNodeState === PAGE_TYPE_STATUS_ENUM.WAIT_DISPATCH
        ...
        render(row) {
          return (
            <>
              ...
              />
              <div>
                {(WBBJ.value && row.isLatest === YES_NO_ENUM.YES && activeTab.value === PAGE_TYPE_STATUS_ENUM.SUBMIT)
                && (
                  <el-button
                    type='primary'
