...
import PatternTaskOrder from './pages/order/list/index.vue';
import PatternTaskExternal from './pages/external/list/index.vue';
import PatternTaskInternal from './pages/internal/list/index.vue';
...
const defaultMenu = ASIDE_TYPE_ENUM.PAPER_TASK_ORDER;
...
const componentMap: Record<ASIDE_TYPE_ENUM, DefineSetupFnComponent<any>> = {
  [ASIDE_TYPE_ENUM.PAPER_TASK_ORDER]: PatternTaskOrder,
  [ASIDE_TYPE_ENUM.PAPER_TASK_INSIDE]: PatternTaskInternal,
  [ASIDE_TYPE_ENUM.PAPER_TASK_OUTSIDE]: PatternTaskExternal,
} as const;

const init = async (params?: any) => {
  menusList.value = getMenus();
  await getStepNodeStateCountListAndUpdateMenu(
    menusList.value,
    {
      ...params,
