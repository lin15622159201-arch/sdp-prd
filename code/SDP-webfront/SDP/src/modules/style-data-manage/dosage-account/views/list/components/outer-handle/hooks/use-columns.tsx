...
import {
  CHECK_COUNT_STATE_LIST, CRAFTS_REQUIRE_LIST, CHECK_COUNT_STATE_ENUM
...
export const useColumns = (props: IProps) => {
  ...
  const handleEditItem = async (row: IListItem) => {
    ...
    $router.push({
      ...
      query: {
        type: ASIDE_TYPE_ENUM.OUTER_HANDLE
      },
    });
  };
  const { columns } = useTableColumns<IListItem>(() => [
    ...
    },
    {
      ...
      render(row) {
        ...
        const getLabel = () => {
          if (row.checkCountReceiving === YES_NO_ENUM.NO) {
            return '待接单';
          }
          return filters.getEnumLabel(CHECK_COUNT_STATE_LIST, row.checkCountState!);
        };
      ...
      render: (row) => {
        ...
        return (
          <>
            ...
            {(row.checkCountReceiving === YES_NO_ENUM.NO
      ...
      render(row) {
        return (
          <div>
            ...
            />
            <div class='tw-flex'>
              {activeConfig.value === CONFIG_TYPE_ENUM.CALCULATE
                && row.isLatest === YES_NO_ENUM.YES
                && BJ.value && (
                <el-button
                  type='primary'
