...
export default defineComponent({
  props: {
    ...
    data: {
      type: Object as PropType<IGroupPageListItem>,
      required: true,
    },
    isEdit: {
      type: Boolean,
  ...
  setup(props, { emit }) {
    ...
    const title = computed(() => {
      return `纸样组别 ${isEdit.value ? '修改' : '新增'}`;
    });
    ...
    // 请求接口，新增或修改
    const requestApi = () => {
      const { groupName, groupId } = formData;
      if (isEdit.value) {
        return updateGroup({
          groupId,
          groupName,
    ...
    const setFormData = (val: IGroupPageListItem) => {
      formData.groupId = val.groupId;
    ...
    const handleConfirm = async () => {
      ...
      formRef.value.resetFields();
      setFormData(defaultData as IGroupPageListItem);
      emit('refreshList');
    };
    watch(
      () => props.data,
    ...
    return {
      ...
      title,
      rules: {
        groupName: [{ required: true, message: '请输入纸样组别', trigger: ['blur'] }],
      },
      YES_NO_STRING_ENUM,
    };
  },
