...
export const useDetailListColumns = ({ detailBillInfo, abnormalDataMap, isView }: IPrams) => {
  // 更新异常原因
  const updateReason = (row: IListItem) => {
    nextTick(() => {
      if (row.abnormalFlag === YES_NO_NUMBER_ENUM.YES && row.detailId) {
        abnormalDataMap[row.detailId] = row.abnormalReason || '';
      }
    });
  };

  // 更新异常状态
  const updateAbnormal = (row: IListItem) => {
    nextTick(() => {
      if (row.abnormalFlag === YES_NO_NUMBER_ENUM.YES && row.detailId) {
        abnormalDataMap[row.detailId] = abnormalDataMap[row.detailId] || ''; // 默认空原因
      } else {
        delete abnormalDataMap[row.detailId]; // 取消异常，删除数据
      }
    });
  };
  // 剪版
    ...
    return [
      {
        ...
        render(row) {
          return (
            <>
              <p>{row.purchaseOrderNo}</p>
              {row.handleAbnormal === YES_NO_NUMBER_ENUM.YES && (
                <el-tag type='danger'>异常重推</el-tag>
              )}
            </>
          );
        }
      ...
      },
      {
        ...
        render(row) {
          return (
            <>
              <div>
                {row.abnormalFlag === YES_NO_NUMBER_ENUM.YES ? '是' : '否'}
                <el-switch
                  v-model={row.abnormalFlag}
                  active-value={1}
                  inactive-value={0}
                  onChange={updateAbnormal(row)}
                  disabled={isView.value}
                />
              </div>
              {row.abnormalFlag === YES_NO_NUMBER_ENUM.YES && (
                <el-input
                  type='textarea'
                  rows={2}
                  maxlength={100}
