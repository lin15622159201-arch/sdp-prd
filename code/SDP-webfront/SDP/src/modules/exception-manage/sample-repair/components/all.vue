  ...
  <sc-app-page>
    ...
    <template #main>
      ...
      >
        ...
        <el-table-column prop="versionNum" min-width="150px">
          <template #header>
            样衣<br>版本
          </template>
        </el-table-column>
        <el-table-column label="图片" min-width="110px">
          <template #default="{ row }">
            <ImageViewer
        ...
        <el-table-column prop="repairNum" min-width="150px">
          <template #header>
            返修<br>件数
          </template>
        </el-table-column>
        ...
        <el-table-column label="二次工艺&裁剪方法" min-width="150px">
          <template #default="{ row }">
            <div class="desc-lis">
        ...
        >
          <template #default="{ row }">
            <div class="flex flex-dir-column flex-justify-between">
              <!-- <span>
                <b>时效</b>：<span v-html="handleAgingTime({row,currentTimeKey:'currentLocalTime'})" />
              </span> -->
              <span>
                <b>当前耗时</b>：<span v-html="handleCostTime({ row })" />
              </span>
            </div>
          </template>
        </el-table-column>
        <el-table-column label="返修环节" min-width="150px">
          <template #default="{ row }">
            <div class="flex flex-dir-column flex-justify-between">
        ...
        >
          <template #default="{ row }">
            <remark-record
              ...
              desc-key="remark"
              :time-formatter="$filters.formatTime"
...
import useAllSearchConfig from '../hooks/use-all-search-config';
import UserSelect from '@/modules/common/components/user-select';
import { DESIGN_DEV_TYPE_LIST, CRAFTS_REQUIRE_LIST, REPAIR_STATE_ALL_LIST } from '../constant';
...
const { allSearchConfig } = useAllSearchConfig();
