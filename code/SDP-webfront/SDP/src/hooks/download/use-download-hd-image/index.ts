...
import { highDefinitionObtain } from './api';
import { TASK_MODE, TASK_STATUS_ENUM } from './constant';
import axios, { CancelTokenSource } from 'axios';
import { IHighDefinitionObtainReq, IHighDefinitionObtainRes } from './api/type';
...
type Props = {
  handleSuccess?: (data: IHighDefinitionObtainRes) => void;
...
export type LoopReq = IHighDefinitionObtainReq & {
  picIndex?: number;
};

export function useDownloadHDImage({
  handleSuccess,
  handleError,
  ...
  const initReq = (): LoopReq => {
    return {
      originTaskId: '',
      taskMode: TASK_MODE.DESIGN_MATERIAL,
      pictureId: '',
      pictureUrl: '',
    };
  };
  ...
  const getImgName = () => {
    ...
    return `${originTaskId}-${(picIndex || 0) + 1}-2k`;
  };
  ...
  const loopFn = async () => {
    ...
    try {
      ...
      const { data } = await highDefinitionObtain(req, false, source.token);
